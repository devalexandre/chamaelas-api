...
// driver's Woovi revenue wallet. It's only ever called once the platform
// has actually collected the fare for that specific ride — a "credit"
// (prepaid-balance) ride settles right at completion, since the real money
// arrived back when she topped up; a "pix" (pay-per-ride) settles once its
// own charge confirms, which can be before or after completion (see
// RideHandler.settleDriverEarningIfPaid and WebhookHandler.handleRidePayment).
// A "cash" ride never reaches this function: the driver already has that
// money in hand directly from the passenger, so crediting the wallet too
// would pay her twice for the same ride.
